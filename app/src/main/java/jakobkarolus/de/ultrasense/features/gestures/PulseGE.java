 ...
 */
public class PulseGE implements GestureExtractor{
    ...
    public CalibrationState calibrate(List<Feature> features) {
        ...
        if(f.getIntegral() <= featureWeightMinThr)
            featureWeightMinThr = f.getIntegral();

        if(f.getIntegral() >= featureWeightMaxThr)
            featureWeightMaxThr = f.getIntegral();
